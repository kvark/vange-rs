...
flat varying vec4 v_Pos;
flat varying vec4 v_Rot;
...
void main() {
    ...
    float out_x = (gl_VertexID == 0 ? -1.0 : 1.0);
    ...
    v_Pos = pos + vec4(delta * vel, 0.0);
    v_Rot = rot + vec4(delta * wel, 0.0);
...
void main() {
    Target0 = (
        gl_FragCoord.x < 1.0 ? v_Pos :
        gl_FragCoord.x < 2.0 ? v_Rot :
        gl_FragCoord.x < 3.0 ? v_Linear :
        v_Angular);
}
#endif //FS
